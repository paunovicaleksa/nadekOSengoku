        ...
        switch(a0){
           ...
           /*     __asm__ volatile("mv a1, %0" :: "r" (tcb));
            ...
            case 0x22:{
                ...
                sem->deinit();
                kmem_cache_free(kSemaphore::cache, sem);
    ...
    }else if (scause == 0x8000000000000009UL)
    {
        ...
        if(ret==CONSOLE_IRQ) {
            ...
            if(((*(char*)CONSOLE_STATUS ) & CONSOLE_TX_STATUS_BIT)){
                if(kConsole::ready->getVal()<1) kConsole::ready->signal();

            }
            if(((*(char*)CONSOLE_STATUS ) & CONSOLE_RX_STATUS_BIT)){
                if(kConsole::inputready->getVal()<1) kConsole::inputready->signal();
            }
        }
        plic_complete(ret);
